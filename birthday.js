...
/* ============================================================
   LOVE QUOTES — one per special heart (4 total)
   ============================================================ */
const LOVE_QUOTES = [
  "and then my soul saw you and it kind of went, \u201Coh, there you are. I\u2019ve been looking for you.\u201D",
  "I\u2019ll kiss every one of your flaws until you realise you are loved.",
  "In a sea of people, my eyes will always search for you you\u2019re the one my heart already knows.",
  "You are my favourite hello and my hardest goodbye every single time.",
];

let quoteOpen = false;
function openQuote(idx) {
  quoteText.textContent = LOVE_QUOTES[idx];
  quoteOverlay.hidden = false;
  // next frame so hidden removal registers before the CSS transition
  requestAnimationFrame(() => quoteOverlay.classList.add('is-open'));
  quoteOpen = true;
}
function closeQuote() {
  quoteOverlay.classList.remove('is-open');
  quoteOpen = false;
  // wait for transition out before hiding
  quoteOverlay.addEventListener('transitionend', () => {
    if (!quoteOpen) quoteOverlay.hidden = true;
  }, { once: true });
}
quoteClose.addEventListener('click', closeQuote);
quoteOverlay.addEventListener('click', (e) => { if (e.target === quoteOverlay) closeQuote(); });
document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && quoteOpen) closeQuote(); });
...
/* ============================================================
   TREE CANVAS CLICK — any tap on the tree cycles through the 4 quotes
   ============================================================ */
let quoteIndex = 0;
canvas.style.cursor = 'pointer';
canvas.addEventListener('click', () => {
  if (!window.bdayDone) return;   // only after the tree has fully settled
  openQuote(quoteIndex);
  quoteIndex = (quoteIndex + 1) % LOVE_QUOTES.length;
});
...
if (reduceMotion) {
  ...
  /* ── wish-replay elements ── */
  const wishReplay = $('wishReplay');
  const wrW1chars = splitWord($('wrW1'));
  const wrW2chars = splitWord($('wrW2'));
  const wrUpath = $('wrUpath');
  const giftScene = $('giftScene');
  const giftBack = $('giftBack');

  /* Pre-set wish-replay chars so they're ready to animate */
  gsap.set([...wrW1chars, ...wrW2chars], {
    transformPerspective: 620, transformOrigin: '50% 100%',
    yPercent: 135, rotationX: -82,
  });

  /* Prep underline stroke */
  const wrLen = wrUpath.getTotalLength();
  wrUpath.style.strokeDasharray = wrLen;
  wrUpath.style.strokeDashoffset = wrLen;

  /* ── NEXT button → in-page sequence ── */
  replay.addEventListener('click', () => {
    replay.hidden = true;
    closeQuote();
    treeStop(); // pause canvas rendering to improve performance in the gift scene
    window.pauseTree = true;
    bloom.style.zIndex = '100'; // bring bloom above all scenes for these transitions

    const reach = Math.hypot(W / 2, H / 2);
    const coverScale = (reach * 1.3) / 30;  // scale needed to fill the viewport

    const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });

    /* Phase 1 — bloom expands to cover screen */
    tl.fromTo(bloom, { scale: 0.02, autoAlpha: 1 },
      { scale: coverScale, duration: 0.52, ease: 'power2.in' }, 0);

    /* Phase 2 — while covered: show wish-replay field + reset its state */
    tl.add(() => {
      wishReplay.style.display = 'block';
      gsap.set([...wrW1chars, ...wrW2chars], { yPercent: 135, rotationX: -82 });
      gsap.set('#wrEye', { opacity: 0, y: 10 });
      gsap.set('#wrSub', { opacity: 0, y: 10 });
      gsap.set('#wrBarT', { yPercent: -100 });
      gsap.set('#wrBarB', { yPercent: 100 });
      wrUpath.style.strokeDashoffset = wrLen;
    }, 0.50);

    /* Phase 3 — bloom shrinks → rose wish-field revealed */
    tl.to(bloom, { scale: 0, duration: 0.42, ease: 'power2.out' }, 0.53);
    tl.set(bloom, { autoAlpha: 0 }, 0.97);

    /* Phase 4 — cinema bars in */
    tl.to('#wrBarT', { yPercent: 0, duration: 0.5, ease: 'power2.out' }, 0.58);
    tl.to('#wrBarB', { yPercent: 0, duration: 0.5, ease: 'power2.out' }, 0.58);

    /* Phase 5 — kinetic type */
    tl.to('#wrEye', { opacity: 1, y: 0, duration: 0.45 }, 0.78);
    tl.to(wrW1chars, { yPercent: 0, rotationX: 0, duration: 0.55, stagger: 0.033 }, 0.98);
    tl.to(wrW2chars, { yPercent: 0, rotationX: 0, duration: 0.55, stagger: 0.033 }, 1.38);
    tl.to(wrUpath, { strokeDashoffset: 0, duration: 0.45, ease: 'power2.inOut' }, 1.88);
    tl.to('#wrSub', { opacity: 1, y: 0, duration: 0.45 }, 2.18);

    /* Phase 6 — cinema bars out */
    tl.to('#wrBarT', { yPercent: -100, duration: 0.45, ease: 'power2.in' }, 3.00);
    tl.to('#wrBarB', { yPercent: 100, duration: 0.45, ease: 'power2.in' }, 3.00);

    /* Phase 7 — bloom covers again */
    tl.fromTo(bloom, { scale: 0.02, autoAlpha: 1 },
      { scale: coverScale, duration: 0.52, ease: 'power2.in' }, 3.20);

    /* Phase 8 — while covered: hide wish-replay, show gift scene */
    tl.add(() => {
      wishReplay.style.display = 'none';
      giftScene.style.display = 'block';
    }, 3.73);

    /* Phase 9 — bloom shrinks → gift scene revealed */
    tl.to(bloom, { scale: 0, duration: 0.62, ease: 'power2.out' }, 3.75);
    tl.set(bloom, { autoAlpha: 0 }, 4.40);

    /* Phase 10 — cards spring in */
    tl.fromTo('#giftBouquet',
      { y: 40, scale: 0.85, autoAlpha: 0 },
      { y: 0, scale: 1, autoAlpha: 1, duration: 0.7, ease: 'back.out(1.5)' }, 3.78);
    tl.fromTo('#giftEnvWrap',
      { y: -38, scale: 0.88, rotation: -4, autoAlpha: 0 },
      { y: 0, scale: 1, rotation: -2, autoAlpha: 1, duration: 0.75, ease: 'back.out(1.4)' }, 3.86);
    tl.fromTo('#giftMystery',
      { y: 50, scale: 0.8, autoAlpha: 0, rotation: 5 },
      { y: 0, scale: 1, autoAlpha: 1, rotation: 2, duration: 0.65, ease: 'back.out(1.6)' }, 3.96);

    /* Phase 11 — back button fades in */
    tl.to(giftBack, { opacity: 1, duration: 0.5 }, 4.38);

    /* Phase 12 — drag hints fade after 5s */
    tl.add(() => {
      bloom.style.zIndex = ''; // restore z-index once we are fully in gift scene
      setTimeout(() => {
  ...
  makeGiftDraggable($('giftBouquet'));
  makeGiftDraggable($('giftEnvWrap'));
  makeGiftDraggable($('giftLetter'));
  makeGiftDraggable($('giftMystery'));

  /* ── GIFT SCENE: envelope seal → slide letter out as free card ── */
  let giftEnvOpen = false;
  let letterReleased = false;
  $('giftSeal').addEventListener('click', e => {
    e.stopPropagation();
    if (letterReleased) return;     // already out — don't re-trigger
    giftEnvOpen = true;
    letterReleased = true;
    $('giftEnvelope').classList.add('is-open');
    $('giftEnvHint').classList.add('hidden');

    // Position the letter card on top of the envelope
    const envRect = $('giftEnvWrap').getBoundingClientRect();
    const sceneRect = $('giftScene').getBoundingClientRect();
    const letter = $('giftLetter');
    letter.style.left = (envRect.left - sceneRect.left) + 'px';
    letter.style.top = (envRect.top - sceneRect.top) + 'px';
    letter.style.display = 'block';

    // Slide it up and slightly left, landing in a natural reading position
    gsap.fromTo(letter,
      { y: 60, x: 20, rotation: 3, autoAlpha: 0, scale: 0.9 },
      {
        y: -envRect.height * 0.7, x: -20, rotation: -2, autoAlpha: 1, scale: 1,
        duration: 0.75, ease: 'back.out(1.3)',
        onComplete: () => {
          // Clear the GSAP inline transform so dragging works cleanly with left/top
          const r = letter.getBoundingClientRect();
          const sr = $('giftScene').getBoundingClientRect();
          gsap.set(letter, { clearProps: 'all' });
          letter.style.display = 'block';
          letter.style.left = (r.left - sr.left) + 'px';
          letter.style.top = (r.top - sr.top) + 'px';
          $('giftLetterHint') && $('giftLetterHint').classList.remove('gone');
        }
      }
    );
  });

  /* ── GIFT SCENE: continuous petal spawner ── */
  const petalCols = ['#ffb8c6', '#ffc8d8', '#ffd5df', '#ffb0be', '#ffd0da', '#f9c4d0'];
  let petalTimer = null;
  function spawnGiftPetal() {
    ...
    gsap.fromTo(p,
      ...
      {
        y: -(window.innerHeight * 1.3), rotation: 720, opacity: 0.3,
        duration: dur, ease: 'none',
        onComplete: () => p.remove()
      }
    );
    gsap.to(p, { opacity: 0.3, duration: 0.5, delay: 0.2 });
  }
  petalTimer = setInterval(spawnGiftPetal, 1800);

  /* ── MYSTERY BOX: unlock glow after letter is released ── */
  // The seal handler (above) sets letterReleased = true; we watch for it
  // and add the glow class a moment after the letter animation lands
  const origSealClick = $('giftSeal').onclick;  // preserve if any
  $('giftSeal').addEventListener('click', () => {
    setTimeout(() => $('giftMystery').classList.add('mystery-unlocked'), 950);
  });

  /* ── MYSTERY BOX: tap detection (independent of drag) ── */
  let mystTapX = 0, mystTapY = 0;
  $('giftMystery').addEventListener('pointerdown', e => {
    mystTapX = e.clientX; mystTapY = e.clientY;
  });
  $('giftMystery').addEventListener('pointerup', e => {
    const dist = Math.hypot(e.clientX - mystTapX, e.clientY - mystTapY);
    if (dist < 10 && letterReleased) triggerAskScene();
  });

  /* ── MYSTERY: transition → ask scene ── */
  function triggerAskScene() {
    $('giftMystery').style.pointerEvents = 'none';   // prevent double-tap
    const reach = Math.hypot(W / 2, H / 2);
    const coverScale = (reach * 1.3) / 30;

    bloom.style.zIndex = '100'; // bring above giftScene for transition
    gsap.set(bloom, { autoAlpha: 1 });
    gsap.fromTo(bloom, { scale: 0.02, autoAlpha: 1 },
      {
        scale: coverScale, duration: 0.52, ease: 'power2.in',
        onComplete: () => {
          /* swap scenes */
          giftScene.style.display = 'none';
          const askScene = $('askScene');
          askScene.style.display = 'block';

          /* reset ask scene state */
          noClickCount = 0;
          gsap.set(['#askLine1', '#askLine15', '#askLine2', '#askLine3', '#askBtns'], { opacity: 0, y: 18 });
          gsap.set('#askLine3', { y: 22 });
          gsap.set('#askYes', { scale: 1 });
          const cel = $('askCelebrate');
          cel.style.display = 'none'; cel.style.opacity = '0';
          $('askBtns').style.display = 'flex';

          /* shrink bloom to reveal ask scene, then run sequence */
          gsap.to(bloom, {
            scale: 0, duration: 0.62, ease: 'power2.out',
            onComplete: () => {
              gsap.set(bloom, { autoAlpha: 0 });
              bloom.style.zIndex = ''; // restore
              runAskSequence();
              startAskPetals();
            }
          });
  ...
  /* ── ASK SEQUENCE: text lines then buttons ── */
  function runAskSequence() {
    const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });
    tl.to('#askLine1', { opacity: 1, y: 0, duration: 1.0 }, 0.5);
    tl.to('#askLine15', { opacity: 1, y: 0, duration: 1.0 }, 1.8);
    tl.to('#askLine2', { opacity: 1, y: 0, duration: 1.0 }, 3.5);
    tl.to('#askLine3', { opacity: 1, y: 0, duration: 1.1 }, 5.8);
    tl.to('#askBtns', { opacity: 1, y: 0, duration: 0.75 }, 7.6);
  }
  ...
  $('askNo').addEventListener('click', () => {
    noClickCount++;
    // Yes grows bigger each time No is clicked
    const newScale = 1 + noClickCount * 0.2;
    gsap.to('#askYes', { scale: newScale, duration: 0.45, ease: 'back.out(1.8)' });
    // No button shakes with frustration
    gsap.to('#askNo', {
      keyframes: [
    ...
    // After enough No clicks, start shrinking No button too
    if (noClickCount >= 3) {
      const noScale = Math.max(0.5, 1 - (noClickCount - 2) * 0.12);
      gsap.to('#askNo', { scale: noScale, duration: 0.4, ease: 'power2.out' });
    }
  });
  ...
  $('askYes').addEventListener('click', () => {
    ...
    gsap.to('#askBtns', {
      ...
      onComplete: () => {
        ...
        burstCelebrationPetals();
      }
    });
  });

  /* ── CELEBRATION petal burst on Yes ── */
  const celebCols = ['#ffb8c6', '#ffc8d8', '#ff8fb0', '#ffd5df', '#ffb0be', '#ffeaf0', '#f9c4d0'];
  function burstCelebrationPetals() {
    for (let i = 0; i < 36; i++) {
      setTimeout(() => {
        const p = document.createElement('div');
  ...
  /* ── Ask scene background petals ── */
  function startAskPetals() {
    const iv = setInterval(() => {
      if ($('askScene').style.display === 'none') { clearInterval(iv); return; }
      const p = document.createElement('div');
      const s = 8 + Math.random() * 14;
      p.style.cssText = 'position:absolute;pointer-events:none;';
      p.style.width = p.style.height = s + 'px';
      ...
      gsap.fromTo(p,
        { y: 0, rotation: 0, opacity: 0 },
        {
          y: -(window.innerHeight * 1.3), rotation: 720, opacity: 0.28,
          duration: 7 + Math.random() * 9, ease: 'none',
          onComplete: () => p.remove()
        }
      );
    }, 1800);
  ...
  /* ── GIFT SCENE: back button → return to tree ── */
    ...
    gsap.fromTo(bloom, { scale: 0.02, autoAlpha: 1 },
      {
        ...
        onComplete: () => {
          ...
          giftScene.style.display = 'none';
          giftEnvOpen = false;
          $('giftEnvelope').classList.remove('is-open');
          $('giftEnvHint').classList.remove('hidden');
          document.querySelectorAll('.gift-drag-hint').forEach(h => h.classList.remove('gone'));
          /* reset card positions */
          const bouq = $('giftBouquet'); bouq.style.left = '55%'; bouq.style.top = '6%';
          const env = $('giftEnvWrap'); env.style.left = '6%'; env.style.top = '28%';
          const myst = $('giftMystery'); myst.style.left = '42%'; myst.style.top = '58%';
          /* reset letter */
          const lt = $('giftLetter'); lt.style.display = 'none'; lt.style.left = '6%'; lt.style.top = '28%';
          gsap.set(lt, { clearProps: 'all' }); lt.style.display = 'none';
          giftEnvOpen = false; letterReleased = false;
          $('giftEnvelope').classList.remove('is-open');
          $('giftEnvHint').classList.remove('hidden');
          gsap.set(lt, { clearProps: 'all' });
          /* show tree + replay button */
          window.pauseTree = false;
          treeStart(); // resume canvas rendering
          replay.hidden = false;
          bloom.style.removeProperty('transform');
          gsap.to(bloom, {
            scale: 0, duration: 0.55, ease: 'power2.out',
